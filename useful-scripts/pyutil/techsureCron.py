...
def get_struct_time(time_stamp_int):
    ...
    # tm_wday返回值不对,改为用strftime转换一下,wangtc,20180108
    w = int(time.strftime("%w", st_time))
    return [st_time.tm_min, st_time.tm_hour, st_time.tm_mday, st_time.tm_mon, w]
...
def handle_starnum(val, ranges=(0, 60), res=list()):
    """星号/数字 组合 如 */3"""
    tmp = val.split('/')
    val_step = int(tmp[1])
    if val_step < 1:
...
def main():
    ...
    test_cron = '* */5 */3 */6 * ?'
    test_cron = test_cron.replace('?', '*')
    test_list = test_cron.split(' ')
    new_list = test_list[1:]
    print new_list
    new_cron = " ".join(new_list)
    print new_cron
